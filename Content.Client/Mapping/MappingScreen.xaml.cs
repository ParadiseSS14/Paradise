...
public sealed partial class MappingScreen : InGameScreen
{
    ...
    private CleaningType _decalCleaningType; // Paradise Change - Cleaning
    ...
    public MappingScreen()
    {
        ...
        foreach (var enums in Enum.GetValues<CleaningType>())
        {
            if (enums is CleaningType.Scrub or CleaningType.Wash or CleaningType.Radiation)
                continue;

            DecalCleaningPicker.AddItem(
                Loc.GetString($"cleaningType-{enums.ToString().ToLowerInvariant()}"));
            DecalCleaningPicker.SetItemMetadata(DecalCleaningPicker.ItemCount - 1, enums);
        }
        ...
        // Paradise Change START - Cleaning
        DecalCleaningPicker.OnItemSelected += args =>
        {
            _decalCleaningType = (CleaningType)args.Id;
            UpdateDecal();
        };
        // Paradise Change END - Cleaning
    ...
    private void UpdateDecal()
    {
        ...
        DecalSystem.UpdateDecalInfo(id, _decalColor, _decalRotation, _decalSnap, _decalZIndex, _decalCleaningType);
    }
    ...
    public void SelectDecal(string decalId)
    {
        ...
        if (_decalAuto)
        {
            ...
            _decalCleaningType = decal.CleaningType; // Paradise Change - Cleaning
            _decalSnap = decal.DefaultSnap;

            DecalColorPicker.Color = _decalColor;
            DecalCleaningPicker.SelectId((int)decal.CleaningType); // Paradise Change - Cleaning
