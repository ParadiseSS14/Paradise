...
public sealed partial class InvestigationRecorderSystem : EntitySystem, IInvestigationPositionSource
{
    ...
    public override void Initialize()
    {
        ...
        _cfg.OnValueChanged(CCVars.InvestigationPositionInterval, i => _positionInterval = TimeSpan.FromSeconds(i), true);
        _cfg.OnValueChanged(CCVars.InvestigationPositionEpsilon, epsilon => _positionEpsilon = epsilon, true);
        _cfg.OnValueChanged(CCVars.InvestigationNavMapInterval, i => _navMapInterval = TimeSpan.FromSeconds(i), true);
        _cfg.OnValueChanged(CCVars.InvestigationCharacterInterval, i => _characterInterval = TimeSpan.FromSeconds(i), true);
        _cfg.OnValueChanged(CCVars.InvestigationDirtyInterval, i => _dirtyInterval = TimeSpan.FromSeconds(i), true);
        _cfg.OnValueChanged(CCVars.InvestigationFlushInterval, i => _flushInterval = TimeSpan.FromSeconds(i), true);
        _cfg.OnValueChanged(CCVars.InvestigationStorageDepth, depth => _storageDepth = depth, true);

        SubscribeLocalEvent<GameRunLevelChangedEvent>(OnRunLevelChanged);

        // The run level misses restartround/golobby, which would otherwise never stop recording.
    ...
    /// <summary>
    ///     Paradise has no round started/ended event pair, so both edges are taken off the run level: the
    ///     transition into <see cref="GameRunLevel.InRound"/> is the start, and out of it is the end.
    /// </summary>
    private void OnRunLevelChanged(GameRunLevelChangedEvent ev)
    {
        if (ev.New == GameRunLevel.InRound)
            ...
            StopRecording(_gameTicker.RoundDuration());
    }
    ...
    private void StartRecording()
    {
        ...
        _recorder.StartRound(_gameTicker.RoundId, _gameMap.GetSelectedMap()?.MapName);
