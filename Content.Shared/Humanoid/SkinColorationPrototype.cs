...
/// <summary>
/// Unary coloration strategy that returns plasmaman skin tones, with 0 being lightest and 100 being darkest
/// </summary>
[DataDefinition]
[Serializable, NetSerializable]
public sealed partial class PlasmamanTonedSkinColoration : ISkinColorationStrategy
{
    [DataField]
    public Color ValidPlasmamanSkinTone = Color.FromHsv(new Vector4(0.80f, 0.2f, 1f, 1f));
    ...
    public bool VerifySkinColor(Color color, [NotNullWhen(false)] out string? reason)
    {
        ...
        // rangeOffset makes it so that this value
        // is 280 <= hue <= 300
        if (hue < 280f || hue > 300f)
        {
            reason = $"Hue {hue} is outside of expected ranges 280 and 300.";
            return false;
        }
    ...
    public Color FromUnary(float color)
    {
        // 0 - 100, 0 being purple and 100 being dark
        // HSV based
    ...
    public float ToUnary(Color color)
    {
        ...
        // and value is 1.0
        // then it'll be hue
        if (Math.Clamp(hsv.X, 280f / 360f, 1) > 280f / 360f
            && hsv.Z == 1.0)
        {
            return Math.Abs(300 - (hsv.X * 360));
        }
        // otherwise it'll directly be the saturation
        else
        {
